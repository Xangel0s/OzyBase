/**
 * OzyBase JS SDK
 * A lightweight client for interacting with OzyBase BAAS
 */
export class OzyBase {
    constructor(baseUrl) {
        this.baseUrl = baseUrl.replace(/\/$/, '');
        this.token = localStorage.getItem('OzyBase_auth_token') || null;
        this.user = JSON.parse(localStorage.getItem('OzyBase_auth_user')) || null;
    }
        ...
        return {
            ...
            login: async (email, password) => {
                ...
                if (res.token) {
                    ...
                    this.user = res.user;
                    localStorage.setItem('OzyBase_auth_token', res.token);
                    localStorage.setItem('OzyBase_auth_user', JSON.stringify(res.user));
                }
                return res;
            },
            logout: () => {
                this.token = null;
                this.user = null;
                localStorage.removeItem('OzyBase_auth_token');
                localStorage.removeItem('OzyBase_auth_user');
            },
            isValid: () => !!this.token
        };
    }
