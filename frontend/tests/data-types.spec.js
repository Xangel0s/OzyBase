...
test('should show the redesigned project overview actions', async ({ page }) => {
    await expect(page.getByText(/Project overview/i)).toBeVisible({ timeout: 30000 });
    await expect(page.getByText(/Primary Database/i)).toBeVisible({ timeout: 30000 });
    await expect(page.getByRole('button', { name: /Open tables/i })).toBeVisible({ timeout: 30000 });
    await expect(page.getByRole('button', { name: /Open SQL/i }).first()).toBeVisible({ timeout: 30000 });
    await expect(page.getByRole('button', { name: /Infrastructure/i })).toBeVisible({ timeout: 30000 });
});
