...
test('massive table workflows stay usable in table editor and sql editor', async ({ page }) => {
    ...
    const initialRangePattern = new RegExp(`1-100 / ${formattedRowCount.replace(',', ',?')}\\+? records`, 'i');
    ...
    try {
        ...
        await expect(footer.getByText(initialRangePattern)).toBeVisible({ timeout: 20000 });
        await expect(footer.getByText('of 12')).toBeVisible({ timeout: 20000 });
        ...
        const pageJump = footer.locator('input').first();
        await pageJump.fill('10');
        await pageJump.press('Enter');
        await expect(pageJump).toHaveValue('10', { timeout: 10000 });
        ...
        await expect(page.getByRole('button', { name: /Saved Views/i })).toBeVisible({ timeout: 20000 });
        await page.getByRole('button', { name: new RegExp(tableName, 'i') }).first().click();
        await expect(page.getByTestId('table-editor-footer')).toBeVisible({ timeout: 20000 });
        ...
        await page.getByTestId('table-editor-footer').getByRole('button', { name: 'Definition' }).click();
