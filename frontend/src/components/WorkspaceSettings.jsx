...
const WorkspaceSettings = ({ workspaceId, view = 'ws_general', onViewSelect, onWorkspaceChange }) => {
    ...
    const fetchData = React.useCallback(async () => {
        ...
        try {
            ...
            if (resWs.ok) {
                ...
                const current = data.find(w => String(w.id) === String(workspaceId));
                if (current) {
    ...
    const handleInvite = async () => {
        ...
        // In a real app, you'd find user by email first, but for now we'll mock it or use ID if known.
        // For simplicity, we'll assume the user enters a valid ID or the backend handles email lookup.
        try {
            const res = await fetchWithAuth(`/api/workspaces/${workspaceId}/members`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ user_id: inviteEmail, role: inviteRole })
            });
            if (res.ok) {
                setInviteEmail('');
                fetchData();
