...
const Integrations = ({ page = 'wrappers' }) => {
    const [pgExtensions, setPgExtensions] = useState([]);
    const [loading, setLoading] = useState(false);
    const [search, setSearch] = useState('');
    const [webhooks, setWebhooks] = useState([]);
    const [cronData, setCronData] = useState({ enabled: false, jobs: [] });
    const [secrets, setSecrets] = useState([]);

    const [activeWrappers, setActiveWrappers] = useState([]);
    ...
    const [showModal, setShowModal] = useState(null); // 'webhook', 'cron', 'secret'
    ...
    const filteredExtensions = pgExtensions.filter(e =>
                ...
                return (
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {wrappers.map((w) => {
                                const isActive = activeWrappers.some(aw => aw.name.toLowerCase().includes(w.id.toLowerCase()));
                                return (
                ...
                return (
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                            {(search ? filteredExtensions : filteredExtensions.slice(0, 50)).map((ext) => (
                ...
                return (
                    <div className="space-y-6">
                        <div className="flex items-center justify-between">
                            ...
                            <button className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 text-zinc-300 px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:border-primary/50 transition-all shadow-xl">
                                <ExternalLink size={14} /> Open Playground (GraphiQL)
                            </button>
                        </div>
                        ...
                        <div className="bg-[#111111] border border-[#2e2e2e] rounded-3xl p-8 shadow-2xl relative overflow-hidden">
                            ...
                            <div className="flex items-center justify-between mb-8 relative z-10">
                                ...
                                {pgExtensions.find(e => e.name === 'pg_graphql')?.installed ? (
                            ...
                            <div className="bg-[#0c0c0c] p-6 rounded-2xl border border-zinc-800/50 font-mono text-[11px] text-zinc-400 group relative">
                                ...
                                http://localhost:8090/graphql/v1
                                <button className="absolute right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity text-[10px] font-black uppercase text-zinc-500 hover:text-white">Copy URL</button>
                            </div>
