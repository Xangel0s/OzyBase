...
const RealtimeInspector = ({ view = 'inspector' }) => {
    ...
    const fetchCollections = useCallback(async () => {
        ...
        try {
            ...
            setCollections(data.filter(c => !c.is_system));
    ...
    const toggleRealtime = async (name, currentStatus) => {
        try {
            const token = localStorage.getItem('ozy_token');
    ...
    const runDiagnostic = async () => {
        ...
        try {
            ...
            const targetTable = collections.find(c => c.realtime_enabled)?.name || 'users';
    ...
    const renderTabs = () => (
        <div className="flex px-6 pt-4 gap-8">
            {['inspector', 'channels', 'configuration'].map(tab => (
                <button
                    key={tab}
                            ...
                            <div className="mt-12 p-6 bg-indigo-500/5 border border-indigo-500/20 rounded-3xl">
                                ...
                                <div className="flex gap-4">
                                    <div className="flex-1 bg-[#111111] border border-zinc-800 rounded-xl p-4">
                                        ...
                                        <p className="text-xl font-black text-white italic">{collections.filter(c => c.realtime_enabled).length}</p>
                                    </div>
                                    <div className="flex-1 bg-[#111111] border border-zinc-800 rounded-xl p-4">
