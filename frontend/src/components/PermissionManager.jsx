...
const PermissionManager = () => {
    ...
    const updateRule = async (colName, type, value) => {
        setSaving(colName);
        try {
                            ...
                            {[
                                ...
                                <div key={rule.id} className="space-y-4">
                                    ...
                                    <div className="relative">
                                        <select
                                            value={col[rule.id] || 'admin'}
                                            onChange={(e) => updateRule(col.name, rule.id, e.target.value)}
                                    ...
                                    <p className="text-[9px] text-zinc-600 font-medium italic">
                                        {col[rule.id] === 'public' ? 'Everyone can access' :
                                            col[rule.id] === 'auth' ? 'Any logged user access' :
                                                `Only ${col[rule.id]} can access`}
                                    </p>
                                </div>
                            ))}
                        </div>
