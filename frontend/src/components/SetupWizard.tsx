...
type MigrationReviewView = 'alerts' | 'preview' | 'sql';
...
interface SetupWizardProps {
    onComplete: (payload: { token: string; workspaceId: string | null; workspaceName: string | null }) => void;
}
...
const MigrationTablePreviewCard: React.FC<{ table: MigrationPreviewTable; showSql?: boolean }> = ({ table, showSql = true }) => {
    ...
    return (
        <div className="rounded-4xl border border-zinc-800 bg-zinc-950/70 p-5 shadow-[0_24px_80px_-48px_rgba(0,0,0,0.9)] space-y-5">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                        <span className="rounded-full border border-blue-500/20 bg-blue-500/10 px-3 py-1 text-[10px] font-medium] text-blue-100/80">
                            {table.display_name}
                        </span>
                        {table.has_more_rows && (
                    ...
                    <h3 className="text-xl font-bold tracking-tight text-white break-all">{table.name}</h3>
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:min-w-[320px]">
                    <div className="rounded-md border border-zinc-800 bg-black/35 px-4 py-3">
                        <span className="block text-[10px] font-medium] text-zinc-500">Columns</span>
                        <span className="mt-1 block text-lg font-bold text-white">{table.column_count}</span>
                    </div>
                    <div className="rounded-md border border-zinc-800 bg-black/35 px-4 py-3">
                        <span className="block text-[10px] font-medium] text-zinc-500">Rows</span>
                        <span className="mt-1 block text-lg font-bold text-white">{table.detected_rows}</span>
                    </div>
                    <div className="rounded-md border border-zinc-800 bg-black/35 px-4 py-3 col-span-2 sm:col-span-1">
                        <span className="block text-[10px] font-medium] text-zinc-500">Preview</span>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 2xl:grid-cols-3">
                {table.columns.map((column) => (
                    <div key={`${table.name}-${column.name}`} className="rounded-md border border-zinc-800 bg-black/35 px-4 py-3">
            ...
            {sampleRows.length > 0 ? (
                <div className="rounded-md border border-zinc-800 bg-black/35 overflow-hidden">
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-left">
                            <thead className="bg-zinc-950/70">
                                <tr>
                                    {table.columns.map((column) => (
                                        <th key={`${table.name}-head-${column.name}`} className="px-4 py-3 text-[10px] font-medium] text-zinc-500">
                                            {column.name}
                                        </th>
                                    ))}
                                </tr>
                ...
                <div className="rounded-md border border-dashed border-zinc-800 bg-black/20 px-4 py-5 text-sm text-zinc-500">
            ...
            {table.warnings && table.warnings.length > 0 && (
                <div className="space-y-2">
                    {table.warnings.map((warning) => (
                        <div key={`${table.name}-${warning}`} className="rounded-md border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-xs text-amber-100/85">
                            {warning}
                        </div>
                    ))}
                </div>
            ...
            {showSql && (
                <details className="rounded-md border border-zinc-800 bg-black/30">
                    <summary className="cursor-pointer list-none px-4 py-3 text-xs font-medium] text-zinc-400">
...
const MigrationPreviewWorkspace: React.FC<MigrationPreviewWorkspaceProps> = ({ preview }) => {
    const tablesWithWarnings = preview.tables.filter((table) => (table.warnings?.length || 0) > 0).length;
    const hasSampleRows = preview.tables.some((table) => (table.sample_rows?.length || 0) > 0);
    const [activeView, setActiveView] = useState<MigrationReviewView>(() => (
    ...
    return (
        <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden px-5 py-5 sm:px-7 sm:py-6">
            <div className="flex flex-wrap gap-2">
                {reviewViews.map((view) => (
                    ...
                    >
                        <span className={`block text-[10px] font-medium] ${activeView === view.id ? 'text-black/70' : 'text-zinc-500'}`}>
                            {view.label}
                        </span>
            ...
            <div className="mt-4 grid min-h-0 flex-1 gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
                <aside className="flex min-h-0 flex-col overflow-hidden rounded-[1.75rem] border border-zinc-800 bg-black/20 p-4">
                    <div className="mb-4 shrink-0">
                        <span className="block text-[10px] font-medium] text-zinc-500">Tables</span>
                    ...
                    <div className="min-h-0 flex-1 overflow-y-auto pr-1 space-y-2">
                        {preview.tables.map((table) => (
                            <button
                                ...
                                className={`w-full rounded-md border px-3 py-3 text-left transition-all ${selectedTable?.name === table.name ? 'border-primary/35 bg-primary/10' : 'border-zinc-800 bg-zinc-950/70 hover:border-primary/20'}`}
                            >
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    {(table.warnings?.length || 0) > 0 && (
                                        <span className="rounded-full border border-amber-500/20 bg-amber-500/10 px-2 py-1 text-[10px] font-medium] text-amber-100/80">
                        ...
                        <div className="space-y-4">
                            {(preview.warnings?.length || 0) === 0 && tablesWithWarnings === 0 ? (
                                <div className="rounded-[1.75rem] border border-emerald-500/20 bg-emerald-500/5 p-5">
                                    <span className="block text-[10px] font-medium] text-emerald-100/80">No warnings</span>
                                ...
                                <>
                                    {preview.warnings && preview.warnings.length > 0 && (
                                        <div className="rounded-[1.75rem] border border-amber-500/20 bg-amber-500/5 p-5">
                                            <div className="flex items-center justify-between gap-3">
                                                <span className="block text-[10px] font-medium] text-amber-100/70">Global alerts</span>
                                                <span className="text-[10px] font-medium] text-amber-100/70">
                                                    {preview.warnings.length} notes
                                                </span>
                                            </div>
                                            <div className="mt-4 space-y-2">
                                                {preview.warnings.map((warning) => (
                                                    <div key={warning} className="rounded-md border border-amber-500/20 bg-black/20 px-3 py-3 text-sm leading-relaxed text-amber-100/85">
                                                        {warning}
                                                    </div>
                                                ))}
                                            </div>
                                    ...
                                    <div className="rounded-[1.75rem] border border-zinc-800 bg-black/20 p-5">
                                        <div className="flex items-center justify-between gap-3">
                                            <span className="block text-[10px] font-medium] text-zinc-500">Table alerts</span>
                                        ...
                                        <div className="mt-4 space-y-3">
                                            {preview.tables.filter((table) => (table.warnings?.length || 0) > 0).map((table) => (
                                                <div key={`table-warning-${table.name}`} className="rounded-md border border-zinc-800 bg-zinc-950/70 p-4">
                                                    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                                                        ...
                                                        <div className="flex gap-2">
                                                            <button
                                                                ...
                                                                className="inline-flex items-center justify-center rounded-md border border-zinc-700 bg-black/20 px-3 py-2 text-[10px] font-medium] text-white transition-all hover:border-primary/35 hover:text-primary"
                                                            >
                                                                Open preview
                                                            </button>
                                                            <button
                                    ...
                                    <div className="rounded-[1.75rem] border border-zinc-800 bg-black/20 p-5">
                                        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                                            <div className="max-w-2xl">
                                                <span className="block text-[10px] font-medium] text-primary">Visual preview</span>
                                            ...
                                            <button
                                                ...
                                                className="inline-flex items-center justify-center rounded-md border border-zinc-700 bg-zinc-950 px-4 py-2 text-[10px] font-medium] text-white transition-all hover:border-primary/35 hover:text-primary"
                                            >
                                                View translated SQL
                                            </button>
                                        </div>
                            ...
                            {selectedTable ? (
                                <>
                                    <div className="rounded-[1.75rem] border border-zinc-800 bg-black/20 p-5">
                                        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                                            <div className="max-w-2xl">
                                                <span className="block text-[10px] font-medium] text-primary">Translated SQL</span>
                                                <h4 className="mt-2 text-lg font-semibold text-white">{selectedTable.name}</h4>
                                                <p className="mt-2 text-sm leading-relaxed text-zinc-400">
                                    ...
                                    {selectedTableWarnings.length > 0 && (
                                        <div className="rounded-md border border-amber-500/20 bg-amber-500/5 px-4 py-4">
                                            <span className="block text-[10px] font-medium] text-amber-100/70">Related alerts</span>
                                    ...
                                    <div className="overflow-hidden rounded-[1.75rem] border border-zinc-800 bg-zinc-950/70">
                                        <div className="border-b border-zinc-800 px-4 py-3">
                                            <span className="text-[10px] font-medium] text-zinc-500">SQL code</span>
                                        </div>
    ...
    return (
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/75 p-3 backdrop-blur-md sm:p-5">
            <div
                ...
                className={`flex min-h-0 w-full flex-col overflow-hidden rounded-4xl border border-zinc-800 bg-background shadow-[0_40px_160px_-64px_rgba(0,0,0,0.95)] ${detailsExpanded ? 'h-[min(88vh,780px)] max-w-[min(90vw,980px)]' : 'max-h-[88vh] max-w-[min(86vw,780px)]'}`}
            >
                <div className="flex flex-col gap-4 border-b border-zinc-800 bg-[linear-gradient(135deg,rgba(254,254,0,0.1),rgba(7,7,7,0.98)_30%,rgba(7,7,7,0.98))] px-5 py-5 sm:px-7 sm:py-6">
                    <div className="flex items-start justify-between gap-4">
                        <div className="max-w-3xl">
                            <div className="mb-3 flex items-center gap-2 text-primary">
                                <ScanSearch size={16} />
                                <span className="text-[10px] font-medium]">Migration Result</span>
                            </div>
                            <h3
                                id="migration-preview-modal-title"
                                className={`font-bold tracking-tight text-white ${detailsExpanded ? 'text-xl sm:text-[1.7rem]' : 'text-2xl sm:text-[2rem]'}`}
                            >
                                {detailsExpanded ? 'Detailed review' : 'Quick review before you continue'}
                            </h3>
                            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-zinc-400">
                                {detailsExpanded
                                    ? 'Focus on warnings, sample rows, and translated SQL without the summary cards getting in the way.'
                        ...
                        <button
                            type="button"
                            onClick={onClose}
                            className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-md border border-zinc-700 bg-black/35 text-zinc-300 transition-all hover:border-primary/35 hover:text-primary"
                    ...
                    {!detailsExpanded && (
                        <div className="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1.8fr)_minmax(0,0.9fr)_minmax(0,1fr)]">
                            <div className="rounded-md border border-primary/20 bg-black/30 px-4 py-4">
                                <span className="block text-[10px] font-medium] text-zinc-500">Summary</span>
                                <p className="mt-2 text-sm font-semibold leading-relaxed text-white">{preview.summary}</p>
                            </div>
                            <div className="rounded-md border border-zinc-800 bg-black/30 px-4 py-4">
                                <span className="block text-[10px] font-medium] text-zinc-500">Source</span>
                                <span className="mt-2 block text-sm font-semibold text-white">{sourceLabel}</span>
                            </div>
                            <div className="rounded-md border border-zinc-800 bg-black/30 px-4 py-4">
                                <span className="block text-[10px] font-medium] text-zinc-500">Snapshot</span>
                                <div className="mt-3 grid grid-cols-2 gap-3">
                                    <div>
                                        <span className="block text-[10px] font-medium] text-zinc-600">Tables</span>
                                        <span className="mt-2 block text-2xl font-bold text-white">{preview.table_count}</span>
                                    </div>
                                    <div>
                                        <span className="block text-[10px] font-medium] text-zinc-600">Rows</span>
                                        <span className="mt-2 block text-2xl font-bold text-white">{preview.row_count}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col gap-3 rounded-[1.35rem] border border-zinc-800 bg-black/20 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <span className="block text-[10px] font-medium] text-zinc-500">Detailed review</span>
                            <p className="mt-2 text-sm text-zinc-400">
                                {detailsExpanded
                                    ? 'Warnings, sample rows, and translated SQL only.'
                                    : 'Expand only when you want warnings, sample rows, or SQL.'}
                            </p>
                        </div>
                        <button
                            type="button"
                            onClick={() => setDetailsExpanded((prev) => !prev)}
                            className="inline-flex items-center justify-center gap-2 rounded-md border border-zinc-700 bg-zinc-950 px-4 py-3 text-[10px] font-medium] text-white transition-all hover:border-primary/35 hover:text-primary"
                        >
                ...
                {detailsExpanded && (
                    <div className="min-h-0 flex-1 overflow-hidden border-b border-zinc-800 animate-in fade-in slide-in-from-bottom-3 duration-300">
                        <MigrationPreviewWorkspace preview={preview} />
                    </div>
                )}

                <div className={`flex flex-col gap-3 bg-background px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7 sm:py-5 ${detailsExpanded ? 'border-t border-zinc-800' : ''}`}>
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                        <button
                            type="button"
                            onClick={onClose}
                            className="inline-flex items-center justify-center rounded-md border border-zinc-700 bg-zinc-950 px-5 py-3 text-[10px] font-medium] text-white transition-all hover:border-primary/35 hover:text-primary"
                        >
                            Keep editing
                        </button>
                        <button
                            type="button"
                            onClick={onContinue}
                            className="inline-flex items-center justify-center gap-2 rounded-md bg-primary px-5 py-3 text-[10px] font-medium] text-black transition-all hover:scale-[1.01]"
                        >
                            Continue
                            <ArrowRight size={14} />
                        </button>
...
const SetupWizard: React.FC<SetupWizardProps> = ({ onComplete }) => {
    ...
    const loadingStepCount = selectedMode?.prepSteps.length || 0;
    const loadingProgress = loadingStepCount > 0
        ? Math.max(
            16,
            Math.min(
                100,
                ((Math.min(loadingPhase, loadingStepCount - 1) + 1) / loadingStepCount) * 100,
            ),
        )
        : 0;
    ...
    const openMigrationFileBrowser = () => {
        migrationFileInputRef.current?.click();
    };

    const handleMigrationUploadZoneKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
        if (event.key === 'Enter' || event.key === ' ') {
            event.preventDefault();
            openMigrationFileBrowser();
        }
    };
    ...
    const handleSetup = async () => {
        ...
        try {
            const res = await fetchWithAuth('/api/system/setup', {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    workspace_name: formData.projectName.trim() || 'Primary Project',
            ...
            if (data.token) {
                onComplete({
                    token: data.token,
                    workspaceId: typeof data.workspace_id === 'string' && data.workspace_id.trim() ? data.workspace_id : null,
                    workspaceName: typeof data.workspace_name === 'string' && data.workspace_name.trim() ? data.workspace_name : null,
                });
            } else {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-stretch justify-center bg-black/90 p-3 backdrop-blur-sm animate-in fade-in duration-500 sm:p-5 md:items-center">
            <div className="relative flex h-full w-full max-w-[min(92vw,1280px)] min-h-0 flex-col overflow-hidden rounded-4xl border border-zinc-800 bg-background shadow-2xl md:h-[min(88vh,820px)] md:max-h-[calc(100vh-2.5rem)] lg:flex-row">
                <div className="relative flex max-h-[30vh] w-full flex-col justify-between overflow-y-auto border-b border-zinc-800 bg-zinc-900/50 p-5 sm:p-6 lg:max-h-none lg:w-74 lg:border-b-0 lg:border-r lg:p-7 xl:w-[20rem]">
                    <div className="absolute inset-0 bg-linear-to-br from-primary/5 via-primary/2 to-transparent pointer-events-none" />

                    <div className="relative">
                        <div className="w-12 h-12 bg-primary rounded-md flex items-center justify-center mb-6 shadow-[0_0_30px_-5px_rgba(254,254,0,0.28)]">
                            <Database className="text-black" size={24} strokeWidth={2.5} />
                        </div>
                        <h1 className="text-3xl font-bold text-white uppercase tracking-tighter italic mb-2">
                    ...
                    <div className="relative space-y-6">
                        ...
                        {selectedMode && (
                            <div className="rounded-md border border-zinc-800 bg-black/30 p-5 animate-in fade-in duration-300">
                                <div className="flex items-start gap-3">
                                    <div className={`p-3 rounded-md ${selectedMode.iconPanelClass}`}>
                ...
                <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden p-4 sm:p-5 lg:p-6 xl:p-7">
                    {loading && selectedMode && (
                        <div className="absolute inset-0 z-20 ozy-overlay-backdrop flex items-center justify-center p-5 md:p-8">
                            <div className="ozy-dialog-panel w-full max-w-lg border border-zinc-800 bg-background/95 px-6 py-7 sm:px-8 sm:py-8">
                                <div className="flex flex-col items-center text-center">
                                    <div className="ozy-loading-logo mb-5 flex h-20 w-20 items-center justify-center rounded-[1.75rem] bg-primary text-black">
                                        <Database size={34} strokeWidth={2.6} />
                                    </div>

                                    <div className="space-y-3">
                                        <p className="text-[10px] font-medium] text-primary/85">
                                            Setup Engine
                                        </p>
                                        <div className="flex flex-wrap items-center justify-center gap-3">
                                            <h3 className="ozy-loading-wordmark text-4xl font-medium] italic sm:text-[2.8rem]">
                                                OzyBase
                                            </h3>
                                            <span className="rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-[10px] font-medium] text-primary/90">
                                                Loading
                                            </span>
                                        </div>
                                        ...
                                        <p className="mx-auto max-w-md text-sm leading-relaxed text-zinc-400">
                                            We are applying the selected setup path so your first login lands inside a ready OzyBase project.
                                        </p>
                                    </div>
                                </div>

                                <div className="mt-6">
                                    <div className="mb-2 flex items-center justify-between gap-3 text-[10px] font-medium] text-zinc-500">
                                        <span>Progress</span>
                                        <span>
                                            {Math.min(loadingPhase + 1, loadingStepCount)}/{loadingStepCount}
                                        </span>
                                    </div>
                                    <div className="overflow-hidden rounded-full border border-zinc-800 bg-zinc-950/80">
                                        <div
                                            className="h-2 rounded-full bg-[linear-gradient(90deg,rgba(254,254,0,0.95),rgba(254,254,0,0.55),rgba(255,255,255,0.75))] transition-all duration-500"
                                ...
                                <div className="mt-6 space-y-3">
                                    {selectedMode.prepSteps.map((label, index) => {
                                        ...
                                        return (
                                            <div
                                                key={label}
                                                className={`rounded-md border px-4 py-3 flex items-center gap-3 transition-all duration-300 ${isComplete ? 'border-primary/30 bg-primary/5' : isCurrent ? 'border-zinc-700 bg-zinc-900/70 shadow-[0_0_0_1px_rgba(254,254,0,0.06)]' : 'border-zinc-800 bg-black/30 text-zinc-600'}`}
                                            >
                                                {isComplete ? (
                    ...
                    <div className="mx-auto flex min-h-0 w-full max-w-[1040px] flex-1 flex-col">
                        {step === 'mode' && (
                            <div className="animate-in slide-in-from-right duration-500 flex-1 min-h-0 overflow-y-auto pr-1">
                                <h2 className="text-2xl font-bold text-white uppercase tracking-tight mb-2">How do you want to start?</h2>
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    {(Object.entries(modeDetails) as Array<[SetupMode, ModeDescriptor]>).map(([modeKey, descriptor]) => (
                                        <button
                                            ...
                                            className={`group p-6 rounded-md border text-left transition-all hover:scale-[1.01] relative overflow-hidden ${descriptor.accentClass}`}
                                        >
                                            <div className="flex items-center justify-between mb-4 gap-4">
                                                <div className={`p-3 rounded-md ${descriptor.iconPanelClass}`}>
                                                    <descriptor.icon size={20} className={descriptor.iconClass} />
                                                </div>
                                                {descriptor.badge ? (
                                                    <span className="px-3 py-1 bg-primary text-black text-[10px] font-medium rounded-full">
                                                        {descriptor.badge}
                                                    </span>
                                                ) : (
                    ...
                    {step === 'prepare' && selectedMode && mode === 'migrate' && (
                        <div className="animate-in slide-in-from-right duration-500 flex-1 min-h-0 overflow-y-auto pr-1">
                            ...
                            <div className="mb-6">
                                ...
                                <h2 className="text-2xl font-bold text-white uppercase tracking-tight mb-2">{selectedMode.prepTitle}</h2>
                            ...
                            <div className="mb-5 grid grid-cols-1 gap-3 xl:grid-cols-3">
                                {migrationPrepareStages.map((stage, index) => {
                                    ...
                                    return (
                                        <div
                                            key={stage.id}
                                            className={`rounded-md border px-4 py-3 transition-all ${isActive
                                        ...
                                        >
                                            <div className="flex items-start gap-3">
                                                <div className={`mt-0.5 flex h-8 w-8 items-center justify-center rounded-full border ${isActive || isComplete ? 'border-primary/40 bg-primary/15 text-primary' : 'border-zinc-700 text-zinc-500'}`}>
                                                    {isComplete ? <CheckCircle size={15} /> : <span className="text-[11px] font-bold">{index + 1}</span>}
                                                </div>
                                                <div>
                                                    <p className={`text-[10px] font-medium] ${isActive ? 'text-primary' : 'text-zinc-500'}`}>{stage.label}</p>
                            ...
                            <div className="pb-2">
                                <div className="rounded-4xl border border-zinc-800 bg-zinc-900/30 p-6 flex flex-col">
                                    <div className="flex items-center gap-3 mb-5">
                                        ...
                                        <div className="hidden">
                                            <p className="text-[10px] font-medium] text-zinc-500 mb-1">Step 1 · Source</p>
                                            <h3 className="text-lg font-bold text-white">Choose the source, load it, then analyze</h3>
                                            <p className="text-xs text-zinc-500">This step only prepares the migration. The detailed review opens after analysis so the project setup stays lighter.</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className={migrationStage === 'upload' ? 'mb-6 grid grid-cols-1 gap-4' : 'hidden'}>
                                        <div className="hidden">
                                            ...
                                            <select
                                                ...
                                                className="w-full rounded-md border border-zinc-800 bg-zinc-900 px-4 py-3 text-sm text-white transition-all focus:border-primary/50 focus:outline-none"
                                            >
                                            ...
                                            <div className="flex items-start gap-4">
                                                ...
                                                <div className="min-w-0">
                                                    <div className="flex flex-wrap items-center gap-2">
                                                        <span className="text-[10px] font-medium] text-primary">{selectedMigrationSource.label}</span>
                                        ...
                                        {selectedMigrationSource.requiresTableName ? (
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    type="text"
                                                    className="w-full rounded-md border border-zinc-800 bg-zinc-900 px-4 py-3 text-white transition-all focus:border-primary/50 focus:outline-none"
                                            ...
                                            <details className="rounded-md border border-zinc-800 bg-black/15">
                                                <summary className="cursor-pointer list-none px-4 py-3 text-[10px] font-medium] text-zinc-400">
                                        ...
                                        <div className="rounded-[1.75rem] border border-zinc-800 bg-black/20 p-5">
                                            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                                <div>
                                                    <p className="text-[10px] font-medium] text-zinc-500 mb-2">Step 2 · Provide the source</p>
                                                ...
                                                <div className="inline-flex rounded-md border border-zinc-800 bg-zinc-950/70 p-1">
                                                    <button
                                                        type="button"
                                                        onClick={() => setMigrationInputMode('upload')}
                                                        className={`rounded-md px-4 py-2 text-[10px] font-medium] transition-all ${migrationInputMode === 'upload' ? 'bg-primary text-black' : 'text-zinc-400 hover:text-white'}`}
                                                    >
                                                        Upload file
                                                    </button>
                                                    <button
                                                        ...
                                                        className={`rounded-md px-4 py-2 text-[10px] font-medium] transition-all ${migrationInputMode === 'paste' ? 'bg-primary text-black' : 'text-zinc-400 hover:text-white'}`}
                                                    >
                                                        Paste text
                                                    </button>
                                                </div>
                                            ...
                                            <div className="mt-4">
                                                {migrationInputMode === 'upload' ? (
                                                    <div
                                                        role="button"
                                                        tabIndex={0}
                                                        aria-label={`Open file browser for ${selectedMigrationSource.label}`}
                                                        onClick={openMigrationFileBrowser}
                                                        onKeyDown={handleMigrationUploadZoneKeyDown}
                                                        onDragOver={(event) => {
                                                            event.preventDefault();
                                                        ...
                                                        className={`cursor-pointer rounded-md border border-dashed p-4 transition-all focus:outline-none focus:ring-2 focus:ring-primary/30 ${migrationDragActive ? 'border-primary bg-primary/8' : 'border-zinc-700 bg-black/20'}`}
                                                    >
                                                        <input
                                                        ...
                                                        <div className="flex flex-col gap-4">
                                                            ...
                                                            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                                                <div className="rounded-md border border-zinc-800 bg-black/30 px-3 py-3 text-xs">
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="flex items-center justify-center gap-2 rounded-md border border-zinc-700 bg-zinc-900 px-4 py-2 text-[10px] font-medium] text-white transition-all hover:border-primary/40 hover:text-primary"
                                                                >
                                                    ...
                                                    <div className="space-y-2">
                                                        ...
                                                        <textarea
                                                            className="min-h-[320px] w-full resize-y rounded-md border border-zinc-800 bg-zinc-900 px-4 py-4 text-sm text-white transition-all focus:border-primary/50 focus:outline-none"
                                    ...
                                    <div className={migrationStage === 'source' ? 'grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3' : 'hidden'}>
                                        {migrationSourceOptions.map((source) => (
                                            <button
                                                ...
                                                className={`rounded-md border p-5 text-left transition-all hover:scale-[1.01] ${migrationDraft.sourceKind === source.id ? 'border-primary/40 bg-primary/8 shadow-[0_0_0_1px_rgba(254,254,0,0.08)]' : source.accentClass}`}
                                            >
                                                <div className="flex items-start justify-between gap-3 mb-3">
                                                    <div className="p-3 rounded-md bg-black/30 border border-white/5">
                                                ...
                                                <span className="text-[10px] font-medium] text-primary">{source.label}</span>
                                    ...
                                    <div className="hidden">
                                        <div className="space-y-2">
                                            ...
                                            <input
                                                type="text"
                                                className="w-full bg-zinc-900 border border-zinc-800 rounded-md px-4 py-3 text-white focus:border-primary/50 focus:outline-none transition-all"
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div
                                                ...
                                                onDrop={handleMigrationDrop}
                                                className={`cursor-pointer rounded-md border border-dashed p-4 flex flex-col gap-4 transition-all focus:outline-none focus:ring-2 focus:ring-primary/30 ${migrationDragActive ? 'border-primary bg-primary/8' : 'border-zinc-700 bg-black/20'}`}
                                            >
                                                <input
                                                    ref={migrationFileInputRef}
                                                    type="file"
                                                ...
                                                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                                                    ...
                                                    <button
                                                        ...
                                                        className="px-4 py-2 rounded-md border border-zinc-700 bg-zinc-900 text-[10px] font-medium] text-white hover:border-primary/40 hover:text-primary transition-all flex items-center justify-center gap-2"
                                                    >
                                                        <MousePointerClick size={14} />
                                                        Browse file
                                                    </button>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <textarea
                                                className="w-full min-h-[320px] resize-y bg-zinc-900 border border-zinc-800 rounded-md px-4 py-4 text-sm text-white focus:border-primary/50 focus:outline-none transition-all"
                                                placeholder={selectedMigrationSource.id === 'csv'
                                                    ? 'id,name,email\n1,Ana,ana@example.com'
                                                    : selectedMigrationSource.id === 'mongo_json'
                                                        ? '[{\"name\":\"Ana\",\"email\":\"ana@example.com\"}]'
                                    ...
                                    <div className={migrationStage === 'analysis' ? 'mt-5 flex flex-col gap-4 rounded-md border border-zinc-800 bg-black/30 p-4 shadow-[0_20px_70px_-48px_rgba(0,0,0,0.85)]' : 'hidden'}>
                                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                            <div>
                                                <p className="text-[10px] font-medium] text-zinc-500">Step 3 · Analyze</p>
                                                <p className="mt-2 text-sm text-zinc-300">Run the quick check, then open the result modal.</p>
                                            </div>
                                            <span className={`rounded-full border px-3 py-1 text-[10px] font-medium] ${migrationHasInput ? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-200/80' : 'border-zinc-700 bg-black/30 text-zinc-400'}`}>
                                        ...
                                        <div className="flex flex-col gap-3 sm:flex-row">
                                            <button
                                                ...
                                                className="flex items-center justify-center gap-2 rounded-md bg-primary px-6 py-3 text-[10px] font-medium text-black transition-all hover:scale-[1.02] disabled:opacity-50 disabled:hover:scale-100"
                                            >
                                            ...
                                            {migrationPreview && (
                                                <button
                                                    ...
                                                    className="flex items-center justify-center gap-2 rounded-md border border-zinc-700 bg-zinc-950 px-6 py-3 text-[10px] font-medium] text-white transition-all hover:border-primary/35 hover:text-primary"
                                                >
                                                    Reopen review
                                                    <ArrowRight size={14} />
                                                </button>
                                        ...
                                        {migrationPreview && (
                                            <div className="rounded-[1.75rem] border border-primary/20 bg-[linear-gradient(135deg,rgba(254,254,0,0.12),rgba(12,12,12,0.45))] p-4">
                                                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                                    <div className="max-w-2xl">
                                                        <span className="text-[10px] font-medium] text-primary">Result ready</span>
                                                        <p className="mt-2 text-sm font-semibold leading-relaxed text-white">{migrationPreview.summary}</p>
                                                    </div>
                                                    <button
                                                        type="button"
                                                        onClick={() => setIsMigrationPreviewModalOpen(true)}
                                                        className="inline-flex items-center justify-center gap-2 rounded-md border border-primary/30 bg-black/25 px-4 py-3 text-[10px] font-medium] text-white transition-all hover:border-primary hover:text-primary"
                                                    >
                                                        Open result modal
                                                        <ArrowRight size={14} />
                                                    </button>
                                                ...
                                                <div className="mt-4 grid grid-cols-2 gap-3 xl:grid-cols-4">
                                                    <div className="rounded-md border border-zinc-800 bg-black/25 px-4 py-4">
                                                        <span className="block text-[10px] font-medium] text-zinc-500">Tables</span>
                                                        <span className="mt-1 block text-2xl font-bold text-white">{migrationPreview.table_count}</span>
                                                    </div>
                                                    <div className="rounded-md border border-zinc-800 bg-black/25 px-4 py-4">
                                                        <span className="block text-[10px] font-medium] text-zinc-500">Rows</span>
                                                        <span className="mt-1 block text-2xl font-bold text-white">{migrationPreview.row_count}</span>
                                                    </div>
                                                    <div className="rounded-md border border-zinc-800 bg-black/25 px-4 py-4">
                                                        <span className="block text-[10px] font-medium] text-zinc-500">Source</span>
                                                    ...
                                                    <div className="rounded-md border border-zinc-800 bg-black/25 px-4 py-4">
                                                        <span className="block text-[10px] font-medium] text-zinc-500">Warnings</span>
                                    ...
                                    <div className="mt-6 border-t border-zinc-800 pt-5 flex flex-col gap-4">
                                        {error && (
                                            <div className="p-3 bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-medium rounded-md flex items-start gap-2">
                                        ...
                                        {migrationStage === 'source' && (
                                            <div className="rounded-md border border-primary/20 bg-primary/5 px-4 py-4">
                                                <p className="text-[10px] font-medium] text-primary">Start here</p>
                                        ...
                                        {migrationStage === 'upload' && (
                                            <div className="flex flex-col gap-3 sm:flex-row">
                                                <button
                                                    ...
                                                    className="w-full rounded-md border border-zinc-700 bg-zinc-950 px-5 py-3 text-[10px] font-medium] text-white transition-all hover:border-primary/35 hover:text-primary"
                                                >
                                                    Change database
                                                </button>
                                                <button
                                                    ...
                                                    className="w-full rounded-md bg-primary px-5 py-3 text-[10px] font-medium] text-black transition-all hover:scale-[1.01] disabled:opacity-50 disabled:hover:scale-100"
                                                >
                                        ...
                                        {migrationStage === 'analysis' && (
                                            <div className="flex flex-col gap-3">
                                                ...
                                                <button
                                                    ...
                                                    className="w-full px-6 py-3 bg-primary text-black rounded-md text-[10px] font-medium hover:scale-[1.01] transition-all disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center gap-2"
                                                >
                                                    Continue to Admin
                                                    <ArrowRight size={14} />
                                                </button>
                                    ...
                                    <div className="hidden">
                                        ...
                                        <button
                                            onClick={handleAnalyzeMigration}
                                            disabled={migrationPreviewing}
                                            className="px-6 py-3 bg-primary text-black rounded-md text-[10px] font-medium hover:scale-[1.02] transition-all disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center gap-2"
                                        >
                                            {migrationPreviewing ? <Loader2 size={14} className="animate-spin" /> : <ScanSearch size={14} />}
                                            {migrationPreviewing ? 'Analyzing...' : 'Analyze migration plan'}
                                        </button>
                                ...
                                <div className="hidden rounded-4xl border border-zinc-800 bg-background flex-col">
                                    ...
                                    {migrationStage === 'source' ? (
                                        <div className="p-6">
                                            <div className="rounded-[1.75rem] border border-zinc-800 bg-black/20 p-5">
                                                <div className="flex items-start gap-4">
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className="text-[10px] font-medium] text-primary mb-2">Minimal flow</p>
                                                        ...
                                                        <div className="mt-4 flex flex-wrap gap-2">
                                                            {migrationPrepareStages.map((stage, index) => (
                                                                <span key={`source-pill-${stage.id}`} className="rounded-full border border-zinc-700 bg-zinc-950/70 px-3 py-1.5 text-[10px] font-medium] text-zinc-300">
                                                                    {index + 1}. {stage.label}
                                                                </span>
                                                            ))}
                                                        </div>
                                        ...
                                        <div className="p-6">
                                            <div className="rounded-[1.75rem] border border-zinc-800 bg-black/20 p-5">
                                                ...
                                                <div className="space-y-3 text-sm">
                                                    <div className="flex items-center justify-between gap-3 rounded-md border border-zinc-800 bg-zinc-950/60 px-3 py-3">
                                        ...
                                        <div className="p-6 space-y-4">
                                            <div className="rounded-4xl border border-primary/20 bg-[linear-gradient(135deg,rgba(254,254,0,0.12),rgba(12,12,12,0.5))] p-5">
                                                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                                    <div className="max-w-2xl">
                                                        <span className="text-[10px] font-medium] text-primary">Analysis ready</span>
                                                        <p className="mt-3 text-base font-semibold leading-relaxed text-white">{migrationPreview.summary}</p>
                                                    </div>
                                                    <button
                                                        type="button"
                                                        onClick={() => setIsMigrationPreviewModalOpen(true)}
                                                        className="inline-flex items-center justify-center gap-2 rounded-md border border-primary/30 bg-black/30 px-4 py-3 text-[10px] font-medium] text-white transition-all hover:border-primary hover:text-primary"
                                                    >
                                                        Open result modal
                                                        <ArrowRight size={14} />
                                                    </button>
                                                ...
                                                <div className="mt-4 grid grid-cols-2 gap-3 xl:grid-cols-4">
                                                    ...
                                                    <div className="rounded-md border border-zinc-800 bg-black/25 px-4 py-4">
                                                        <span className="block text-[10px] font-medium] text-zinc-500">Setup action</span>
                                            ...
                                            {migrationPreview.warnings && migrationPreview.warnings.length > 0 && (
                                                <div className="rounded-md border border-amber-500/20 bg-amber-500/5 px-4 py-4">
                                                    <div className="flex items-center justify-between gap-3">
                                                        <span className="block text-[10px] font-medium] text-amber-100/70">Warnings</span>
                                                        <span className="text-[10px] font-medium] text-amber-100/70">
                                                    ...
                                                    <div className="mt-3 space-y-2">
                                                        {migrationPreview.warnings.slice(0, 2).map((warning) => (
                                                            <div key={warning} className="rounded-md border border-amber-500/20 bg-black/20 px-3 py-3 text-xs leading-relaxed text-amber-100/85">
                                                                {warning}
                                                            </div>
                                                        ))}
                                                    </div>
                                            ...
                                            <div className="rounded-md border border-zinc-800 bg-black/20 px-4 py-4">
                                                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                                    <div>
                                                        <span className="block text-[10px] font-medium] text-zinc-500">Preview tables</span>
                                    ...
                                    <div className="border-t border-zinc-800 px-6 py-5 bg-background flex flex-col gap-4">
                                        {error && (
                                            <div className="p-3 bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-medium rounded-md flex items-start gap-2">
                                                <Shield size={14} className="shrink-0 mt-0.5" />
                                                <span>{error}</span>
                    ...
                    {step === 'prepare' && selectedMode && mode !== 'migrate' && (
                        <div className="animate-in slide-in-from-right duration-500 h-full min-h-0 flex flex-col">
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-[1.15fr_0.85fr] gap-6 flex-1 min-h-0">
                                    ...
                                    <div className="space-y-3 flex-1">
                                        {selectedMode.prepSteps.map((prepStep, index) => {
                                            ...
                                            return (
                                                <div
                                                    key={prepStep}
                                                    className={`rounded-md border px-4 py-3 transition-all duration-300 ${isReady ? 'border-primary/30 bg-primary/5' : 'border-zinc-800 bg-black/25'}`}
                                                >
                                ...
                                <div className="rounded-4xl border border-zinc-800 bg-background p-6 flex flex-col justify-between">
                                    <div>
                    ...
                    {step === 'account' && selectedMode && (
                        <div className="animate-in slide-in-from-right duration-500 h-full min-h-0 flex flex-col">
                            ...
                            <h2 className="text-2xl font-bold text-white uppercase tracking-tight mb-2">Register Admin Account</h2>
                            ...
                            <div className="space-y-6 flex-1 min-h-0 overflow-y-auto pr-1">
                                <div className="p-5 rounded-md border border-zinc-800 bg-zinc-900/30">
                                    <div className="flex items-start gap-4">
                                        <div className={`p-3 rounded-md ${selectedMode.iconPanelClass}`}>
                                            <selectedMode.icon size={20} className={selectedMode.iconClass} />
                                        </div>
                                        <div className="min-w-0">
                                            <div className="flex flex-wrap items-center gap-2 mb-2">
                                                ...
                                                {selectedMode.badge && (
                                                    <span className="px-2.5 py-1 rounded-full bg-primary text-black text-[10px] font-medium">
                                                        {selectedMode.badge}
                                                    </span>
                                                )}
                                            </div>
                                    ...
                                    <div className="mt-5 grid grid-cols-1 gap-3 xl:grid-cols-2">
                                        {selectedMode.accountBullets.map((bullet) => (
                                            <div key={bullet} className="flex items-start gap-3 rounded-md border border-zinc-800 bg-black/20 px-4 py-3">
                                    ...
                                    {mode === 'secure' && (
                                        <div className="mt-5 p-4 bg-primary/5 border border-primary/20 rounded-md">
                                    ...
                                    {mode === 'migrate' && (
                                        <div className="mt-5 rounded-md border border-blue-500/20 bg-blue-500/5 p-4 text-xs leading-relaxed text-blue-100/80">
                                            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                                <div className="max-w-2xl">
                                                    <span className="text-[10px] font-medium] text-blue-200/60">Migration summary</span>
                                                    <p className="mt-2">
                                                ...
                                                {migrationPreview && (
                                                    <button
                                                        type="button"
                                                        onClick={() => setIsMigrationPreviewModalOpen(true)}
                                                        className="inline-flex items-center justify-center gap-2 rounded-md border border-blue-500/20 bg-black/20 px-4 py-2 text-[10px] font-medium] text-white transition-all hover:border-primary/35 hover:text-primary"
                                                    >
                                                        Open reviewed plan
                                                        <ArrowRight size={14} />
                                                    </button>
                                            ...
                                            {migrationPreview && (
                                                <div className="mt-4 space-y-4">
                                                    <div className="grid grid-cols-2 gap-3 text-xs xl:grid-cols-4">
                                                        <div className="rounded-md border border-blue-500/20 bg-black/20 px-3 py-3">
                                                            <span className="mb-1 block text-[10px] uppercase tracking-widest text-blue-200/60">Tables</span>
                                                            <span className="text-base font-bold text-white">{migrationPreview.table_count}</span>
                                                        </div>
                                                        <div className="rounded-md border border-blue-500/20 bg-black/20 px-3 py-3">
                                                            <span className="mb-1 block text-[10px] uppercase tracking-widest text-blue-200/60">Rows</span>
                                                            <span className="text-base font-bold text-white">{migrationPreview.row_count}</span>
                                                        </div>
                                                        <div className="rounded-md border border-blue-500/20 bg-black/20 px-3 py-3">
                                                    ...
                                                    <div className="rounded-md border border-blue-500/20 bg-black/20 px-4 py-4">
                                                        <span className="text-[10px] font-medium] text-blue-200/60">Included tables</span>
                                ...
                                <div className="space-y-4 rounded-4xl border border-zinc-800 bg-zinc-950/60 p-5">
                                    <div>
                                        <p className="text-[10px] font-medium] text-primary mb-2">Project + admin</p>
                                        <h3 className="text-lg font-bold text-white">Create the first active project and admin credentials</h3>
                                        <p className="mt-1 text-sm text-zinc-500">Setup creates this project in OzyBase first, then signs you into that active context immediately.</p>
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-[11px] leading-relaxed text-zinc-500">
                                            This names your project inside OzyBase. It does not rename the PostgreSQL database configured by your deploy.
                                        </p>
                                    </div>
                                    <div className="space-y-2">
                                ...
                                {serverSummary && (
                                    <div className="p-4 rounded-md border border-primary/20 bg-primary/5 text-sm text-zinc-200 animate-in fade-in duration-300">
                                        <div className="flex items-center gap-2 text-primary mb-2">
                                            <CheckCircle size={14} />
                                            <span className="text-[10px] font-medium]">Server Summary</span>
                                        </div>
                                        <p>{serverSummary}</p>
                            ...
                            {error && (
                                <div className="mt-4 p-3 bg-red-500/10 border border-red-500/20 text-red-500 text-xs font-bold rounded-md flex items-center gap-2">
                                    <Lock size={14} /> {error}
                                </div>
                            )}
                            ...
                            <div className="mt-6 pt-6 border-t border-zinc-800 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                                ...
                                <button
                                    onClick={handleSetup}
                                    disabled={loading}
                                    className="w-full xl:w-auto px-8 py-3 bg-primary text-black rounded-md text-[10px] font-medium hover:scale-[1.02] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                                >
