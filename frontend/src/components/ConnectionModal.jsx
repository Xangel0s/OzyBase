...
const ConnectionModal = ({ isOpen, onClose }) => {
    ...
    useEffect(() => {
        ...
        fetchWithAuth('/api/project/connection')
            ...
            .catch((error) => console.error('Failed to fetch project connection:', error))
            ...
            <div className="relative bg-[#1a1a1a] border border-[#2e2e2e] rounded-2xl shadow-2xl w-full max-w-4xl max-h-[85vh] overflow-hidden animate-in zoom-in-95 fade-in duration-200">
                <div className="px-6 py-4 border-b border-[#2e2e2e] flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-zinc-500 mt-1">Safe connection metadata only. Passwords and service-role secrets are never exposed here.</p>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-6">
                            {[
                                ...
                                <div key={key} className="bg-[#111111] p-4 rounded-xl border border-[#2e2e2e]">
                                    <div className="flex items-center justify-between gap-4">
                                        ...
                                        <button onClick={() => copyValue(value, key)} className="p-2 bg-[#1a1a1a] rounded-lg border border-[#2e2e2e] hover:border-primary/50 transition-all text-zinc-400 hover:text-white shrink-0">
                                    ...
                                    <tbody className="divide-y divide-[#2e2e2e]/50 text-xs">
                                        {[
                                            ...
                                            <tr key={label} className="hover:bg-zinc-900/30">
                                                <td className="px-4 py-3 font-bold text-zinc-500 uppercase tracking-widest w-28">{label}</td>
                                                <td className="px-4 py-3 text-zinc-300 font-mono">{value}</td>
                                                <td className="px-4 py-3 text-right">
                                                    <button onClick={() => copyValue(value, label)} className="p-1 text-zinc-600 hover:text-white">
                        ...
                        <div className="space-y-6">
                            <div className="bg-[#111111] p-4 rounded-xl border border-[#2e2e2e]">
                                <div className="flex items-center justify-between gap-4">
                                    <div>
                                        <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500 mb-2">API URL</p>
                                        <code className="text-xs text-zinc-300 break-all">{connection?.api_url}</code>
                                    </div>
                                    <button onClick={() => copyValue(connection?.api_url, 'api-url')} className="p-2 bg-[#1a1a1a] rounded-lg border border-[#2e2e2e] hover:border-primary/50 transition-all text-zinc-400 hover:text-white shrink-0">
                                        {copied === 'api-url' ? <Check size={14} className="text-green-500" /> : <Copy size={14} />}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="bg-[#111111] p-4 rounded-xl border border-[#2e2e2e]">
                                <div className="flex items-center justify-between gap-4">
                                    <div>
                                        <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500 mb-2">Current Session Token</p>
                                        <code className="text-xs text-zinc-300 break-all">{localStorage.getItem('ozy_token') || 'No active session token'}</code>
                                    </div>
                                    <button onClick={() => copyValue(localStorage.getItem('ozy_token') || '', 'session-token')} className="p-2 bg-[#1a1a1a] rounded-lg border border-[#2e2e2e] hover:border-primary/50 transition-all text-zinc-400 hover:text-white shrink-0">
                                        {copied === 'session-token' ? <Check size={14} className="text-green-500" /> : <Copy size={14} />}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="bg-[#111111] p-4 rounded-xl border border-[#2e2e2e]">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <h3 className="text-sm font-bold text-white mb-1">Service Secrets</h3>
                                        <p className="text-xs text-zinc-500 leading-relaxed">
                                            Service-role keys and raw database passwords are intentionally removed from the UI.
                                            Generate dedicated API keys from the Settings screen when you need server-side access.
                                        </p>
                                    </div>
                                </div>
                            </div>
