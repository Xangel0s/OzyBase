...
const normalizeSeries = (data: any, fallbackSize: any = DEFAULT_POINTS) => {
    ...
    return data.map((v: any) => Number(v) || 0);
};
...
const formatCompactMetric = (value: any) => {
    const num = Number(value) || 0;
    if (num < 1000) return `${Math.round(num * 10) / 10}`;
    return new Intl.NumberFormat('en-US', { notation: 'compact', maximumFractionDigits: 1 }).format(num);
};

const formatRangeLabels = (minutes: any) => {
    const safeMinutes = Math.max(1, Number(minutes) || 60);
    const end = new Date();
    const start = new Date(end.getTime() - safeMinutes * 60 * 1000);
    const dateFormat: Intl.DateTimeFormatOptions = { hour: '2-digit', minute: '2-digit', hour12: false };
    return {
...
const formatBytes = (bytesValue: any) => {
...
const getLatestNumericSample = (series: any) => {
...
const BarChart = ({ data = [], color, suffix = 'requests', maxOverride, timeRange = 60 }: any) => {
...
const MetricSparkline = ({ data = [], tone = 'emerald', suffix = 'events', timeRange = 60 }: any) => {
    const [hoveredIndex, setHoveredIndex] = useState<any>(null);
    ...
    const palette = toneMap[tone as keyof typeof toneMap] || toneMap.emerald;
    const points = safeData.map((value: any, idx: any) => {
...
const ModuleCard = ({ icon, title, metricLabel, value, data, tone, signalText, timeRange, metricUnit }: any) => {
    ...
    const average = safeData.reduce((acc: any, val: any) => acc + val, 0) / safeData.length;
    ...
    const toneClass = toneMap[tone as keyof typeof toneMap] || toneMap.emerald;
...
const Overview: React.FC<OverviewProps> = ({ onViewSelect }) => {
    ...
    const securityIssues = useMemo(() => healthIssues.filter((i: any) => i.type === 'security').length, [healthIssues]);
    const performanceIssues = useMemo(() => healthIssues.filter((i: any) => i.type === 'performance').length, [healthIssues]);
            ...
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-12">
                    ...
                    <div className="relative">
                        <button
                            onClick={(e: any) => { e.stopPropagation(); setShowStatusMenu(!showStatusMenu); }}
                ...
                <div className="bg-[#171717] border border-[#2e2e2e] rounded-xl overflow-hidden flex flex-col h-96">
                    <div className="flex items-center border-b border-[#2e2e2e]">
                        ...
                        >
                            Security <span className="bg-amber-500/20 text-amber-500 px-1.5 rounded text-[9px]">
                                {healthIssues.filter((i: any) => i.type === 'security').length}
                            </span>
                        </button>
                        <button
                        ...
                        >
                            Performance <span className="bg-amber-500/20 text-amber-500 px-1.5 rounded text-[9px]">
                                {healthIssues.filter((i: any) => i.type === 'performance').length}
                            </span>
                        </button>
                    ...
                    <div className="p-4 flex-1 overflow-y-auto custom-scrollbar">
                        <div className="space-y-2">
                            {healthIssues.filter((i: any) => i.type === issuesTab).length > 0 ? (
                                healthIssues.filter((i: any) => i.type === issuesTab).map((issue: any, idx: any) => (
                    ...
                    <div className="flex-1 overflow-hidden flex flex-col">
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar">
                            {projectInfo?.slow_queries?.length > 0 ? (
                                projectInfo.slow_queries.map((item: any, i: any) => (
