...
const InlineCellEditor = ({
    ...
    const isPrimaryKeyColumn = Boolean(primaryKeyColumn && columnName === primaryKeyColumn);
    ...
    // Read-only primary keys should never enter edit mode.
    if (isPrimaryKeyColumn) {
        return (
            <span className="font-mono text-[11px] text-zinc-500 select-all cursor-text" onContextMenu={onContextMenu}>
                {type.includes('uuid') ? formatUUIDDisplayValue(value) : String(value ?? '')}
            </span>
        );
    }
    ...
    if (!isEditing) {
        ...
        if (type.includes('bool')) {
            return (
                ...
                >
                    ...
                    <span className={`text-[9px] font-medium ${value ? 'text-primary' : 'text-zinc-600'}`}>
                        {value ? 'True' : 'False'}
                    </span>
                </button>
            );
        ...
        if (type.includes('time') || type.includes('date')) {
            return (
                <span className="font-mono text-[10px] text-zinc-500" onContextMenu={onContextMenu}>
        ...
        if (type.includes('json')) {
            return (
                <span className="font-mono text-[10px] text-zinc-500 truncate max-w-[200px]" onContextMenu={onContextMenu}>
        ...
        return (
            <span className="text-zinc-400 group-hover:text-zinc-200 transition-colors truncate" onContextMenu={onContextMenu}>
                {String(value ?? '')}
            </span>
        );
    }
