...
const SqlResultsHeader: React.FC<SqlResultsHeaderProps> = ({
    ...
    <div className="h-10 border-b border-border flex items-center justify-between px-6 bg-background sticky top-0 z-50">
        <div className="flex items-center gap-4">
            <button
                onClick={() => setActiveTab('results')}
                className={`text-[10px] font-medium transition-all px-4 h-full border-b-2 flex items-center gap-2
            ...
            {results && activeTab === 'results' && (
                <div className="flex items-center gap-6 border-l border-zinc-800 pl-6 h-full">
                    <span className="text-[9px] font-bold text-emerald-500 uppercase tracking-[0.2em] flex items-center gap-2 font-mono">
                        <CheckCircle2 size={10} />
                        {results.message}
                    </span>
                    <div className="flex items-center gap-4 text-[9px] font-bold text-zinc-600 tracking-widest font-mono uppercase">
                        {results.truncated && (
                            <span className="text-amber-500/80">
                                LIMIT: {results.resultLimit}
                            </span>
                        )}
                        <span>
                            TIME: {results.executionTime || '0ms'}
                        </span>
                    </div>
                </div>
            )}
        ...
        <div className="flex items-center gap-4">
            {activeTab === 'results' && hasTabularResults && (
                <div className="relative hidden xl:block">
                    ...
                    <input
                        ...
                        className="w-56 rounded-md border border-border bg-[#111111] py-1.5 pl-8 pr-3 text-[10px] font-bold tracking-widest text-zinc-200 placeholder:text-zinc-600 focus:border-primary/30 focus:outline-none"
                    />
                </div>
            )}
            {activeTab === 'visualize' && results && (
                <div className="relative">
                    <button
                        ...
                        className="flex items-center gap-2 text-[9px] font-bold text-zinc-500 hover:text-white uppercase tracking-widest transition-colors border-r border-border pr-4"
                    >
                        <Clock size={12} />
                        Last {timeRange} mins
                    ...
                    {showTimeMenu && (
                        <div
                            className="absolute right-0 mt-3 w-52 bg-[#131313] border border-border rounded-md shadow-[0_20px_50px_rgba(0,0,0,0.5)] z-100 py-2 animate-in fade-in slide-in-from-top-2 duration-200"
                        ...
                        >
                            {[60, 120, 160, 200].map((minutes) => (
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between px-4 py-2.5 text-[10px] font-medium transition-all
            ...
            <div className="relative">
                ...
                {showExportMenu && (
                    <div className="absolute right-0 mt-2 w-36 bg-[#131313] border border-border rounded-md shadow-xl z-50 py-1 animate-in fade-in slide-in-from-top-1 duration-200">
                        <button
