...
export function shouldRenderExplorerSidebar(viewId: string): boolean {
    if (!shouldShowExplorer(viewId)) {
        return false;
    }

    const moduleId = getExplorerModule(viewId);
    if (moduleId === 'realtime') {
        return false;
    }
    if (moduleId === 'tables' || moduleId === 'database') {
        return true;
    }

    return (SUBMENUS[moduleId] || []).length > 1;
}
