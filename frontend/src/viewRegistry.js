...
export function getExplorerModule(viewId) {
...
export function shouldShowExplorer(viewId) {
    return getViewMeta(viewId).supportsExplorer;
}

export function getDefaultViewForSection(sectionId) {
    const defaults = {
        auth: 'users',
        storage: 'buckets',
        edge: 'functions',
...
export function isPrimaryNavActive(navId, selectedView) {
    if (navId === 'database') {
