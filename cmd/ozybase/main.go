...
func setupEcho(ctx context.Context, h *api.Handler, cfg *config.Config, cronMgr *realtime.CronManager) *echo.Echo {
	...
	mcpRoleAccess := api.RequireAnyRole("admin", "service_role")
	...
	apiGroup.Use(api.LimitsEnforcementMiddleware(h.DB, cfg.LimitsEnforcementV2))
	{
		...
		apiGroup.GET("/project/mcp", h.GetMCPManifest, authRequired, mcpRoleAccess)
		apiGroup.POST("/project/mcp", h.HandleMCPRPC, authRequired, mcpRoleAccess)
		apiGroup.GET("/project/mcp/tools", h.GetMCPTools, authRequired, mcpRoleAccess)
		apiGroup.POST("/project/mcp/invoke", h.InvokeMCPTool, authRequired, mcpRoleAccess)
		apiGroup.GET("/project/mcp/approvals/pending", h.ListPendingMCPApprovals, authRequired, adminOnly)
		apiGroup.POST("/project/mcp/approvals/action", h.ResolveMCPApproval, authRequired, adminOnly)
		apiGroup.GET("/project/mcp/skills", h.ListMCPSkills, authRequired, adminOnly)
		apiGroup.POST("/project/mcp/skills/action", h.UpdateMCPSkill, authRequired, adminOnly)
		apiGroup.POST("/project/mcp/stream/session", h.CreateMCPStreamSession, authRequired, adminOnly)
		apiGroup.GET("/project/mcp/stream", h.StreamMCPEvents)
		apiGroup.PUT("/project/mcp/agents/:id", h.UpdateAgentConfig, authRequired, adminOnly)
		apiGroup.GET("/project/engram/config", h.GetEngramConfig, authRequired, adminOnly)
		apiGroup.POST("/project/engram/config", h.SaveEngramConfig, authRequired, adminOnly)
		apiGroup.GET("/project/engram/autonomy", h.GetEngramAutonomyConfig, authRequired, adminOnly)
		apiGroup.POST("/project/engram/autonomy", h.SaveEngramAutonomyConfig, authRequired, adminOnly)
		apiGroup.POST("/project/engram/config/test", h.TestEngramSync, authRequired, adminOnly)
		apiGroup.POST("/project/engram/compact-now", h.CompactEngramNow, authRequired, adminOnly)
		apiGroup.POST("/project/engram/respond", h.RespondEngramArchitect, authRequired, adminOnly)
