...
function Test-DockerResponsive {
  ...
  $dockerBinary = Resolve-DockerBinary

  try {
    $process = Start-Process -FilePath $dockerBinary -ArgumentList @("info") -NoNewWindow -PassThru -RedirectStandardOutput $stdout -RedirectStandardError $stderr
    if (-not $process.WaitForExit($TimeoutSeconds * 1000)) {
      Stop-Process -Id $process.Id -Force -ErrorAction SilentlyContinue
      return $false
    }
    $process.Refresh()
    return ($process.HasExited -and $process.ExitCode -eq 0)
  } catch {
    return $false
  }
}
...
function Remove-ContainerSafe {
  ...
  try {
    ...
    $process = Start-Process -FilePath $dockerBinary -ArgumentList @("rm", "-f", $Name) -NoNewWindow -PassThru -RedirectStandardOutput $stdout -RedirectStandardError $stderr
...
function Remove-NetworkSafe {
  ...
  try {
    ...
    $process = Start-Process -FilePath $dockerBinary -ArgumentList @("network", "rm", $Name) -NoNewWindow -PassThru -RedirectStandardOutput $stdout -RedirectStandardError $stderr
    if (-not $process.WaitForExit(30000)) {
      Stop-Process -Id $process.Id -Force -ErrorAction SilentlyContinue
    }
  } catch {
