...
func TestCreatePolicyForAction_RejectsInvalidAction(t *testing.T) {
	...
	err := db.CreatePolicyForAction(context.Background(), nil, "users", "policy_users_test", "merge", "true", nil)
	if err == nil {
...
func TestCreatePolicyForAction_RejectsInvalidIdentifiers(t *testing.T) {
	...
	err := db.CreatePolicyForAction(context.Background(), nil, "users;", "policy_users_test", "select", "true", nil)
	if err == nil {
		t.Fatalf("expected invalid identifier error for table name")
	}

	err = db.CreatePolicyForAction(context.Background(), nil, "users", "policy users", "select", "true", nil)
	if err == nil {
