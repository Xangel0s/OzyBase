...
func (db *DB) InsertAuditLog(ctx context.Context, log AuditLog) error {
	...
	`, log.UserID, log.WorkspaceID, log.IP, log.Method, log.Path, log.Status, log.Latency, log.Country, log.City, log.UserAgent, log.CreatedAt)
	return err
}
...
func (db *DB) BulkInsertAuditLogs(ctx context.Context, logs []AuditLog) error {
	...
	_, err := db.Pool.CopyFrom(
		...
		[]string{"user_id", "workspace_id", "ip_address", "method", "path", "status", "latency_ms", "country", "city", "user_agent", "created_at"},
