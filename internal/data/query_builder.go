...
func (qb *QueryBuilder) Where(column, operator string, value any) *QueryBuilder {
	...
	sqlOp := "="
	colExpr := QuoteIdentifier(column)
	...
	case "like", "ilike":
		sqlOp = "ILIKE"
		colExpr = fmt.Sprintf("%s::text", QuoteIdentifier(column))
...
func (qb *QueryBuilder) Order(orderBy string) *QueryBuilder {
	...
	for _, clause := range clauses {
		...
		safeClauses = append(safeClauses, fmt.Sprintf("%s %s", QuoteIdentifier(column), direction))
	}
...
func (qb *QueryBuilder) BuildSelect() (string, []any) {
	...
	query := fmt.Sprintf("SELECT * FROM %s%s%s%s", QuoteIdentifier(qb.tableName), where, order, limits)
...
func (qb *QueryBuilder) BuildCount() (string, []any) {
	...
	query := fmt.Sprintf("SELECT COUNT(*) FROM %s%s", QuoteIdentifier(qb.tableName), where)
	return query, qb.args
}
