...
func (l *LocalProvider) Move(ctx context.Context, bucket, oldKey, newKey string) error {
	oldPath, err := l.resolvePath(bucket, oldKey)
	if err != nil {
		return err
	}
	newPath, err := l.resolvePath(bucket, newKey)
	if err != nil {
		return err
	}
