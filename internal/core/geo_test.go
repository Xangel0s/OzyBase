...
func TestCheckBreachBlocksConfiguredCountry(t *testing.T) {
	service := &GeoService{
		policyCache: &GeoPolicy{
			Enabled:          true,
			BlockedCountries: []string{"PE"},
		},
	}
	...
	if !isBreach {
		t.Fatalf("expected Peru to be blocked when policy contains PE in denylist mode")
	}
}

func TestCheckBreachPreservesLegacyAllowlistPolicies(t *testing.T) {
	service := &GeoService{
		policyCache: &GeoPolicy{
			Enabled:          true,
	...
	if isBreach {
		t.Fatalf("expected Peru to remain allowed for legacy allowlist policies")
	}
}
