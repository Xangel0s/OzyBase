...
func (s *GeoService) CheckBreach(ctx context.Context, ip string, country string) (bool, error) {
	...
	if len(policy.BlockedCountries) > 0 {
		for _, blockedCountry := range policy.BlockedCountries {
			if geoCountryMatches(blockedCountry, country) {
				return true, nil
			}
		}
		return false, nil
	...
	// Backward compatibility for installations that still persist the legacy allowlist shape.
	if len(policy.AllowedCountries) > 0 {
		for _, allowedCountry := range policy.AllowedCountries {
			if geoCountryMatches(allowedCountry, country) {
				return false, nil
			}
		}
		return true, nil
