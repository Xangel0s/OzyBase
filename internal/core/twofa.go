...
func (s *TwoFactorService) GetStatus(ctx context.Context, userID string) (TwoFactorStatus, error) {
	var status TwoFactorStatus
	var lastUsedAt *time.Time

	err := s.db.Pool.QueryRow(ctx, `
		SELECT is_enabled, last_used_at
		FROM _v_user_2fa
		WHERE user_id = $1
	`, userID).Scan(&status.Enabled, &lastUsedAt)
	if err != nil {
		return status, nil
	}

	status.LastUsedAt = lastUsedAt
	status.PendingFirstProtectedIn = status.Enabled && lastUsedAt == nil
	return status, nil
}
