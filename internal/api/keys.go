...
func (h *Handler) ListAPIKeys(c echo.Context) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&k.ID, &k.Name, &k.Prefix, &k.Role, &k.SecurityLevel, &k.IsActive, &k.ExpiresAt, &k.CreatedAt, &k.LastUsedAt, &workspaceID, &createdBy, &k.KeyGroupID, &k.KeyVersion, &k.RotatedToKeyID, &k.GraceUntil); err != nil {
			continue
		}
		if workspaceID != nil {
