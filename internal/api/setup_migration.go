...
func applySetupMigration(ctx context.Context, tx pgx.Tx, workspaceID string, plan setupMigrationPlan) (setupMigrationApplyResult, error) {
	...
	for _, table := range plan.Tables {
		...
		if exists {
			...
			if err := upsertMigrationCollectionMetadata(ctx, tx, workspaceID, table); err != nil {
				return result, err
			}
			continue
		}
...
func upsertMigrationCollectionMetadata(ctx context.Context, tx pgx.Tx, workspaceID string, table setupMigrationTablePlan) error {
	schemaJSONBytes, err := json.Marshal(table.Schema)
	if err != nil {
	...
	_, err = tx.Exec(ctx, `
		...
		VALUES ($1, $2, $3, 'auth', 'admin', FALSE, 'auth.uid() = owner_id', FALSE, $4, NOW())
		ON CONFLICT (name) DO UPDATE SET
			...
			updated_at = NOW()
	`, table.Name, table.DisplayName, schemaJSON, workspace)
	if err != nil {
...
func execMigrationInsertBatchTx(
	...
	builder.WriteString(data.QuoteIdentifier(tableName))
	builder.WriteString(" (")
	quotedColumns := make([]string, 0, len(columns))
	for _, column := range columns {
		quotedColumns = append(quotedColumns, data.QuoteIdentifier(column))
	}
