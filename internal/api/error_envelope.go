...
const publicErrorFlagKey = "_error_public"
...
func normalizeErrorPayload(c echo.Context, status int, payload interface{}) map[string]any {
	...
	publicError, _ := result[publicErrorFlagKey].(bool)
	delete(result, publicErrorFlagKey)
	...
	if status >= http.StatusInternalServerError && !publicError {
		result["error"] = http.StatusText(http.StatusInternalServerError)
		result["error_code"] = inferErrorCode(status)
	}
