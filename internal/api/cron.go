...
func (h *CronHandler) cronStatus(c echo.Context) (*CronStatusResponse, error) {
	status := &CronStatusResponse{
		Extension: "ozy_native", // Default to native
		Jobs:      []CronJobInfo{},
		Available: true,         // Ozy Native Scheduler is ALWAYS available
	}

	var pgCronAvailable, pgCronEnabled bool
	_ = h.DB.Pool.QueryRow(c.Request().Context(), `
		SELECT EXISTS (SELECT 1 FROM pg_available_extensions WHERE name = 'pg_cron'),
		       EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron')
	`).Scan(&pgCronAvailable, &pgCronEnabled)

	if pgCronAvailable {
		status.Extension = "pg_cron"
		status.Available = true
		status.Enabled = pgCronEnabled
	} else {
		status.Enabled = true // Native engine is always enabled
	}
...
func (h *CronHandler) Logs(c echo.Context) error {
	id := c.Param("id")
	rows, err := h.DB.Pool.Query(c.Request().Context(), `
	...
	for rows.Next() {
		var l_id, status, message string
		var duration int64
		var createdAt time.Time
		if err := rows.Scan(&l_id, &status, &message, &duration, &createdAt); err == nil {
