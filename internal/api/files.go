...
const defaultStorageSignedURLTTL = 5 * time.Minute
const maxStorageSignedURLTTL = 60 * time.Minute
...
type storageSignedURLClaims struct {
	Scope      string `json:"scope"`
...
type storageBucketPolicyRecord struct {
	ID          string
	Action      string
	Effect      string
	Priority    int
...
type storagePolicySubject struct {
	Public         bool   `json:"public,omitempty"`
	Authenticated  bool   `json:"authenticated,omitempty"`
	UserID         string `json:"user_id,omitempty"`
	Role           string `json:"role,omitempty"`
	WorkspaceID    string `json:"workspace_id,omitempty"`
	ObjectOwner    bool   `json:"object_owner,omitempty"`
	PathPrefix     string `json:"path_prefix,omitempty"`
	RequireService bool   `json:"service_role,omitempty"`
}

type storagePolicyEvaluationContext struct {
	Action        string
	UserID        string
	Role          string
	WorkspaceID   string
...
func WithStoragePolicyV2(enabled bool) FileHandlerOption {
	return func(h *FileHandler) {
		h.PolicyV2Enabled = enabled
	}
}

func WithStorageSignedKeyID(keyID string) FileHandlerOption {
	return func(h *FileHandler) {
...
func NewFileHandler(db *data.DB, storageSvc ozystorage.Provider, storageDir string, uploadKey string, opts ...FileHandlerOption) *FileHandler {
	handler := &FileHandler{
		DB:          db,
		Storage:     storageSvc,
...
func (h *FileHandler) Upload(c echo.Context) error {
	...
	if _, err := h.authorizeBucket(c, bucket, storageActionWrite); err != nil {
		h.auditStorageObjectAccess(c, &bucket, nil, "insert", false, err.Error())
	...
	openedSource, err := fileHeader.Open()
	if err != nil {
		return c.JSON(http.StatusBadRequest, map[string]string{
			"error": "Failed to open uploaded file: " + err.Error(),
		})
	}
	var source io.ReadCloser = openedSource
	...
	headerProbe := make([]byte, 512)
	probeLen, probeErr := io.ReadFull(source, headerProbe)
	if probeErr != nil && probeErr != io.EOF && probeErr != io.ErrUnexpectedEOF {
		h.auditStorageObjectAccess(c, &bucket, nil, "insert", false, "failed to inspect uploaded mime")
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Failed to inspect uploaded file"})
	}
	headerProbe = headerProbe[:probeLen]
	detectedContentType := http.DetectContentType(headerProbe)
	if !isCompatibleMIME(contentType, detectedContentType) {
		h.auditStorageObjectAccess(c, &bucket, nil, "insert", false, "mime mismatch")
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Declared content_type does not match uploaded file content"})
	}
	source = io.NopCloser(io.MultiReader(bytes.NewReader(headerProbe), source))
	...
	if err := h.validateWorkspaceStorageLimit(c.Request().Context(), workspacePointerFromContext(c), fileHeader.Size); err != nil {
		h.auditStorageObjectAccess(c, &bucket, nil, "insert", false, err.Error())
		return c.JSON(http.StatusRequestEntityTooLarge, map[string]string{"error": err.Error()})
	}
	...
	payload, err := h.createStoredObjectRecord(c.Request().Context(), bucket, uuidPointerFromContext(c), workspacePointerFromContext(c), displayName, objectKey, fileHeader.Size, contentType)
	if err != nil {
		_ = h.Storage.Delete(c.Request().Context(), bucket.Name, objectKey)
		h.auditStorageObjectAccess(c, &bucket, nil, "insert", false, err.Error())
		return storageUploadMetadataError(c, err)
	}
	h.auditStorageObjectAccess(c, &bucket, nil, "insert", true, "ok")
...
func (h *FileHandler) CreateUploadSession(c echo.Context) error {
	...
	workspaceID := workspacePointerFromContext(c)
	...
	err = h.DB.Pool.QueryRow(c.Request().Context(), `
		...
		RETURNING id
	`, bucket.ID, ownerID, workspaceID, displayName, req.Size, contentType, objectKey, expiresAt).Scan(&sessionID)
	if err != nil {
...
func (h *FileHandler) CreateMultipartUploadSession(c echo.Context) error {
	...
	var sessionID string
	err = h.DB.Pool.QueryRow(c.Request().Context(), `
		INSERT INTO _v_storage_upload_sessions (bucket_id, owner_id, workspace_id, name, size, content_type, storage_key, mode, chunk_size_bytes, expires_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, 'multipart', $8, $9)
		RETURNING id
	`, bucket.ID, ownerID, workspaceID, displayName, req.Size, contentType, objectKey, chunkSize, expiresAt).Scan(&sessionID)
	if err != nil {
...
func (h *FileHandler) CompleteMultipartUpload(c echo.Context) error {
	...
	if err := h.validateWorkspaceStorageLimit(c.Request().Context(), session.WorkspaceID, session.Size); err != nil {
		return c.JSON(http.StatusRequestEntityTooLarge, map[string]string{"error": err.Error()})
	}
	...
	payload, err := h.createStoredObjectRecord(c.Request().Context(), bucket, session.OwnerID, session.WorkspaceID, session.Name, session.StorageKey, session.Size, session.ContentType)
	if err != nil {
...
func (h *FileHandler) UploadStream(c echo.Context) error {
	...
	if err != nil {
		h.auditStorageObjectAccess(c, nil, nil, "insert", false, "invalid upload token")
	...
	if err != nil {
		h.auditStorageObjectAccess(c, nil, nil, "insert", false, err.Error())
		return storageErrorResponse(c, err)
	}
	...
	headerProbe := make([]byte, 512)
	probeLen, probeErr := io.ReadFull(c.Request().Body, headerProbe)
	if probeErr != nil && probeErr != io.EOF && probeErr != io.ErrUnexpectedEOF {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Failed to inspect uploaded stream"})
	}
	headerProbe = headerProbe[:probeLen]
	detectedContentType := http.DetectContentType(headerProbe)
	if !isCompatibleMIME(session.ContentType, detectedContentType) {
		h.auditStorageObjectAccess(c, &bucket, nil, "insert", false, "mime mismatch")
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Declared content_type does not match uploaded stream content"})
	}
	streamReader := io.MultiReader(bytes.NewReader(headerProbe), c.Request().Body)
	...
	if err := h.Storage.Upload(c.Request().Context(), bucket.Name, session.StorageKey, streamReader, session.Size, session.ContentType, storageObjectACL(bucket)); err != nil {
		h.auditStorageObjectAccess(c, &bucket, nil, "insert", false, err.Error())
...
func (h *FileHandler) List(c echo.Context) error {
	...
	query := `
		SELECT id, COALESCE(owner_id::text, ''), COALESCE(workspace_id::text, ''), name, size, content_type, path, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&object.ID, &ownerID, &workspaceID, &object.Name, &object.Size, &object.ContentType, &object.StoragePath, &object.CreatedAt); err != nil {
			continue
		}
...
func (h *FileHandler) Download(c echo.Context) error {
	...
	ownerFilter := ""
	allowViaSignedURL := false
	if signedToken := strings.TrimSpace(c.QueryParam("token")); signedToken != "" {
		claims, tokenErr := h.validateStorageSignedReadToken(signedToken, time.Now().UTC())
		if tokenErr != nil {
			h.auditStorageObjectAccess(c, &bucket, nil, "read", false, "invalid signed url token")
			return c.JSON(http.StatusUnauthorized, map[string]string{"error": "signed url token is invalid or expired"})
		}
		if strings.TrimSpace(claims.BucketName) != bucket.Name || strings.TrimSpace(claims.ObjectPath) != objectKey {
			h.auditStorageObjectAccess(c, &bucket, nil, "read", false, "signed url scope mismatch")
			return c.JSON(http.StatusForbidden, map[string]string{"error": "signed url is not valid for this object"})
		}
		allowViaSignedURL = true
	} else {
		ownerFilter, err = h.authorizeBucket(c, bucket, storageActionRead)
		if err != nil {
			h.auditStorageObjectAccess(c, &bucket, nil, "read", false, err.Error())
			return c.JSON(http.StatusForbidden, map[string]string{"error": err.Error()})
		}
	}
	...
	if !allowViaSignedURL {
		if err := h.enforceObjectPolicyV2(c, bucket, "select", object); err != nil {
			h.auditStorageObjectAccess(c, &bucket, &object, "read", false, err.Error())
			return c.JSON(http.StatusForbidden, map[string]string{"error": err.Error()})
		}
	}
	...
	h.auditStorageObjectAccess(c, &bucket, &object, "read", true, "ok")
...
func (h *FileHandler) DeleteObject(c echo.Context) error {
	...
	if err != nil {
		h.auditStorageObjectAccess(c, &bucket, nil, "delete", false, err.Error())
		return c.JSON(http.StatusForbidden, map[string]string{"error": err.Error()})
	}
	...
	if err := h.enforceObjectPolicyV2(c, bucket, "delete", object); err != nil {
		h.auditStorageObjectAccess(c, &bucket, &object, "delete", false, err.Error())
		return c.JSON(http.StatusForbidden, map[string]string{"error": err.Error()})
	}
	...
	if commandTag.RowsAffected() == 0 {
		h.auditStorageObjectAccess(c, &bucket, &object, "delete", false, "object metadata not found")
		return c.JSON(http.StatusNotFound, map[string]string{"error": "Object not found"})
	}
	h.auditStorageObjectAccess(c, &bucket, &object, "delete", true, "ok")

	return c.JSON(http.StatusOK, map[string]string{"message": "Object deleted"})
}

// MoveObject handles POST /api/files/:bucket/move
func (h *FileHandler) MoveObject(c echo.Context) error {
	bucketName := normalizeBucketName(c.Param("bucket"))
	var req struct {
	...
	object, err := h.findObject(c.Request().Context(), bucket, req.OldPath, ownerFilter)
	if err != nil {
		return storageErrorResponse(c, err)
	}

	if err := h.enforceObjectPolicyV2(c, bucket, "update", object); err != nil {
		return c.JSON(http.StatusForbidden, map[string]string{"error": err.Error()})
	}

	// For simple move/rename, we just update the storage key and DB path
	// In S3, this is copy + delete. In local, it's rename.
	if err := h.Storage.Move(c.Request().Context(), bucket.Name, object.StoragePath, req.NewPath); err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to move file in storage: " + err.Error()})
	}

	// Update DB record
	_, err = h.DB.Pool.Exec(c.Request().Context(), `
		UPDATE _v_storage_objects
		SET path = $1, name = $2, updated_at = NOW()
		WHERE id = $3
	`, req.NewPath, filepath.Base(req.NewPath), object.ID)
	if err != nil {
		// Rollback storage move if DB update fails? 
		// For simplicity, we just return error. Users can retry.
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to update object metadata: " + err.Error()})
	}

	return c.JSON(http.StatusOK, map[string]string{"message": "Object moved successfully"})
}
...
func (h *FileHandler) authorizeBucket(c echo.Context, bucket bucketRecord, action storageAction) (string, error) {
	if h.PolicyV2Enabled {
		...
		decision, err := h.evaluateStoragePolicyV2(c, bucket, policyAction, "", "")
		if err != nil {
			return "", err
		}
		if decision.Resolved {
			...
			return decision.OwnerFilter, nil
		}
	}
	return h.authorizeBucketLegacy(c, bucket, action)
}

func (h *FileHandler) authorizeBucketLegacy(c echo.Context, bucket bucketRecord, action storageAction) (string, error) {
...
func (h *FileHandler) createStoredObjectRecord(ctx context.Context, bucket bucketRecord, ownerID *string, workspaceID *string, displayName, objectKey string, size int64, contentType string) (map[string]any, error) {
	var objectID string
	err := h.DB.Pool.QueryRow(ctx, `
		INSERT INTO _v_storage_objects (bucket_id, owner_id, workspace_id, name, size, content_type, path)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
		RETURNING id
	`, bucket.ID, ownerID, workspaceID, displayName, size, contentType, objectKey).Scan(&objectID)
	if err != nil {
		return nil, err
	}
...
func isCompatibleMIME(declared, detected string) bool {
	...
	if declaredMajor == detectedMajor && declaredMajor != "" {
		return true
	}
	if strings.HasPrefix(declared, "text/") && detected == "application/json" {
		return true
	}
	if declared == "application/json" && strings.HasPrefix(detected, "text/") {
		return true
	}
	return false
}
...
func workspacePointerFromContext(c echo.Context) *string {
