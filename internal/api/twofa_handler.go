...
func (h *TwoFactorHandler) Get2FAStatus(c echo.Context) error {
	...
	return c.JSON(http.StatusOK, map[string]any{
		...
		"pending_first_protected_in":   status.PendingFirstProtectedIn,
		"challenge_applies_next_login": status.Enabled,
	})
}
...
func (h *TwoFactorHandler) Verify2FA(c echo.Context) error {
	...
	err = h.authService.DB().Pool.QueryRow(c.Request().Context(), `
		...
		FROM _v_users
		WHERE id = $1
	`, req.UserID).Scan(&user.ID, &user.Email, &user.Role, &user.IsVerified, &user.CreatedAt, &user.UpdatedAt)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "user not found"})
	}

	// Generate full JWT, session, and refresh token.
	pair, err := h.authService.GenerateSessionTokenPair(c.Request().Context(), req.UserID, user.Role, c.RealIP(), c.Request().UserAgent(), true)
	if err != nil {
