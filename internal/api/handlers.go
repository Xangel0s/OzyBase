...
func NewHandler(db *data.DB, broker *realtime.Broker, webhooks *realtime.WebhookDispatcher, mailSvc mailer.Mailer, storageSvc storage.Provider, ps realtime.PubSub, migrator *migrations.Generator, applier *migrations.Applier, production ProjectProductionReadiness, audit *core.AuditService) *Handler {
	...
	go h.StartCollectionAutoDiscovery(context.Background())

	return h
}

func collectionAutoDiscoveryEnabled() bool {
	raw := strings.ToLower(strings.TrimSpace(os.Getenv("OZY_COLLECTION_AUTODISCOVERY")))
	if raw == "" {
		return true
	}
	switch raw {
...
func collectionAutoDiscoveryPruneEnabled() bool {
	raw := strings.ToLower(strings.TrimSpace(os.Getenv("OZY_COLLECTION_AUTODISCOVERY_PRUNE")))
	return raw == "1" || raw == "true" || raw == "on" || raw == "yes"
}

func collectionAutoDiscoveryInterval() time.Duration {
	raw := strings.TrimSpace(os.Getenv("OZY_COLLECTION_AUTODISCOVERY_INTERVAL_SECONDS"))
	if raw == "" {
...
func (h *Handler) StartCollectionAutoDiscovery(ctx context.Context) {
	if h == nil || h.DB == nil || h.DB.Pool == nil {
		return
	}
	if !collectionAutoDiscoveryEnabled() {
		return
	}

	interval := collectionAutoDiscoveryInterval()
	pruneMissing := collectionAutoDiscoveryPruneEnabled()
...
func (h *Handler) runCollectionAutoDiscoveryPass(parentCtx context.Context, pruneMissing bool) error {
	ctx, cancel := context.WithTimeout(parentCtx, 12*time.Second)
	defer cancel()

	sqlTables, err := h.loadUserSQLTables(ctx)
	if err != nil {
		return err
	}
	collections, err := h.loadUserCollectionNames(ctx)
	if err != nil {
		return err
	}
	...
	if pruneMissing {
		for name := range collections {
			if _, exists := sqlTables[name]; exists {
				continue
			}
...
func (h *Handler) loadUserSQLTables(ctx context.Context) (map[string]struct{}, error) {
	rows, err := h.DB.Pool.Query(ctx, `
		SELECT t.table_name
		FROM information_schema.tables t
		WHERE t.table_schema = 'public'
		  AND t.table_type = 'BASE TABLE'
		  AND t.table_name NOT LIKE '_v_%'
		  AND t.table_name NOT LIKE '_ozy_%'
	`)
	if err != nil {
		return nil, err
	}
...
func (h *Handler) loadUserCollectionNames(ctx context.Context) (map[string]struct{}, error) {
	rows, err := h.DB.Pool.Query(ctx, `
		SELECT name
		FROM _v_collections
		WHERE name NOT LIKE '_v_%'
		  AND name NOT LIKE '_ozy_%'
	`)
	if err != nil {
		return nil, err
	}
...
type HealthResponse struct {
	...
	Pulse     struct {
		...
		Database HealthPulseComponent `json:"database"`
		Realtime HealthPulseComponent `json:"realtime"`
		Workers  HealthPulseComponent `json:"workers"`
		Storage  HealthPulseComponent `json:"storage"`
	} `json:"pulse"`
	SLO struct {
...
type HealthPulseComponent struct {
	Status    string `json:"status"`
...
func (h *Handler) Health(c echo.Context) error {
	...
	resp.Pulse.Database = HealthPulseComponent{
		Status:    classifyHealthStatusByLatency(dbCheck, 100),
		LatencyMS: dbCheck.LatencyMS,
		Detail:    "postgresql pool check",
	}
	resp.Pulse.Storage = HealthPulseComponent{
		Status:    classifyHealthStatusByLatency(storageCheck, 100),
		LatencyMS: storageCheck.LatencyMS,
		Detail:    "storage provider health",
	}
	...
	if h.Broker == nil {
		realtimeStatus = "fail"
		realtimeDetail = "broker unavailable"
	} else {
		subscribers = h.Broker.ClientCount()
		realtimeDetail = fmt.Sprintf("broker active, subscribers=%d", subscribers)
	}
	resp.Pulse.Realtime = HealthPulseComponent{
		Status:    realtimeStatus,
		LatencyMS: time.Since(realtimeStart).Milliseconds(),
	...
	if h.Audit == nil || !h.Audit.IsRunning() {
		workersStatus = "fail"
		workersDetail = "audit worker stopped"
	} else {
		auditQueueDepth = h.Audit.QueueDepth()
		if auditQueueDepth > 1000 {
			workersStatus = "warning"
			workersDetail = fmt.Sprintf("audit worker active, queue_depth=%d", auditQueueDepth)
		} else {
			workersDetail = fmt.Sprintf("audit worker active, queue_depth=%d", auditQueueDepth)
		}
	}
	resp.Pulse.Workers = HealthPulseComponent{
		Status: workersStatus,
		Detail: workersDetail,
	}

	resp.Pulse.Overall = "ok"
	if resp.Pulse.Database.Status == "fail" || resp.Pulse.Realtime.Status == "fail" || resp.Pulse.Workers.Status == "fail" || resp.Pulse.Storage.Status == "fail" {
		resp.Pulse.Overall = "fail"
	} else if resp.Pulse.Database.Status == "warning" || resp.Pulse.Realtime.Status == "warning" || resp.Pulse.Workers.Status == "warning" || resp.Pulse.Storage.Status == "warning" {
		resp.Pulse.Overall = "warning"
	}
...
func classifyHealthStatusByLatency(check HealthCheck, warningThresholdMS int64) string {
	if check.Status != "ok" {
		return "fail"
	}
	if warningThresholdMS > 0 && check.LatencyMS > warningThresholdMS {
		return "warning"
	}
	return "ok"
}
		...
		if err := rows.Scan(&pType, &config); err == nil {
			...
			if pType == "geo_fencing" {
				if geoConfig, ok := configMap.(map[string]any); ok {
					blockedCountries := normalizeAllowedCountries(geoConfig["blocked_countries"])
					if len(blockedCountries) == 0 {
						blockedCountries = normalizeAllowedCountries(geoConfig["allowed_countries"])
					}
					geoConfig["blocked_countries"] = blockedCountries
					delete(geoConfig, "allowed_countries")
					configMap = geoConfig
				}
			}
...
func (h *Handler) UpdateSecurityPolicy(c echo.Context) error {
	...
	if req.Type == "geo_fencing" {
		req.Config["blocked_countries"] = normalizeAllowedCountries(req.Config["blocked_countries"])
		delete(req.Config, "allowed_countries")
	}
...
func (h *Handler) GetNotificationRecipients(c echo.Context) error {
	rows, err := h.DB.Pool.Query(c.Request().Context(), `
		SELECT id::text, email, COALESCE(to_json(alert_types)::text, '[]'), is_active, created_at
	...
	for rows.Next() {
		var r Recipient
		var rawAlertTypes string
		var createdAt time.Time
		if err := rows.Scan(&r.ID, &r.Email, &rawAlertTypes, &r.IsActive, &createdAt); err == nil {
			_ = json.Unmarshal([]byte(rawAlertTypes), &r.AlertTypes)
...
func (h *Handler) AddNotificationRecipient(c echo.Context) error {
	...
	if _, parseErr := mail.ParseAddress(strings.TrimSpace(req.Email)); parseErr != nil {
	...
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "failed to add notification recipient"})
	}
...
func (h *Handler) DeleteNotificationRecipient(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "failed to delete notification recipient"})
	}
