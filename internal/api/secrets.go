...
func (h *Handler) CreateSecret(c echo.Context) error {
	...
	if mailer.IsManagedSMTPSecretKey(input.Key) {
		return c.JSON(http.StatusForbidden, publicErrorPayload("SMTP settings are managed from Auth Settings.", "RESERVED_SECRET_KEY"))
	}
...
func (h *Handler) DeleteSecret(c echo.Context) error {
	...
	var key string
	if err := h.DB.Pool.QueryRow(ctx, "SELECT key FROM _v_secrets WHERE id = $1", id).Scan(&key); err == nil && mailer.IsManagedSMTPSecretKey(key) {
		return c.JSON(http.StatusForbidden, publicErrorPayload("SMTP settings are managed from Auth Settings.", "RESERVED_SECRET_KEY"))
	}
