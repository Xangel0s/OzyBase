...
const setupDefaultWorkspaceName = "Primary Project"
...
func (h *Handler) SetupSystem(c echo.Context) error {
	...
	if req.WorkspaceName == "" {
		req.WorkspaceName = setupDefaultWorkspaceName
	}
	...
	workspace, err := core.CreateWorkspaceTx(c.Request().Context(), tx, req.WorkspaceName, userID)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to create initial workspace"})
	}
	...
	case "migrate":
		migrationResult, err = applySetupMigration(c.Request().Context(), tx, workspace.ID, migrationPlan)
		if err != nil {
			return c.JSON(http.StatusBadRequest, map[string]string{"error": err.Error()})
		}
	...
	return c.JSON(http.StatusOK, buildSetupResponse(req.Mode, token, workspace, allowedCountries, preservedTableCount, migrationResult))
}

func buildSetupResponse(mode, token string, workspace *core.Workspace, allowedCountries []string, preservedTableCount int, migrationResult setupMigrationApplyResult) setupSystemResponse {
	workspaceID := ""
	workspaceName := setupDefaultWorkspaceName
	if workspace != nil {
		workspaceID = strings.TrimSpace(workspace.ID)
		if trimmed := strings.TrimSpace(workspace.Name); trimmed != "" {
	...
	actions := []setupAction{
		...
		},
		{
			...
			Label:  "Primary project created",
			Detail: fmt.Sprintf("Workspace bootstrap created %s as the initial active project.", workspaceName),
		},
	}
	...
	switch mode {
	case "secure":
		geoDetail := "Geo-fencing was enabled in blocklist mode. All countries remain allowed until you explicitly add blocked regions."
		summary = "Admin bootstrap completed with the secure preset."
		if len(allowedCountries) > 0 {
			geoDetail = fmt.Sprintf("Geo-fencing was enabled in blocklist mode. %s was detected as the operator region for onboarding context, but it was not blocked automatically.", allowedCountries[0])
			summary = fmt.Sprintf("Admin bootstrap completed with geo-fencing enabled in blocklist mode. Operator region detected: %s.", allowedCountries[0])
		}
