...
import (
	...
	"github.com/Xangel0s/OzyBase/internal/buildinfo"
...
func (h *Handler) GetProjectConnection(c echo.Context) error {
	...
	connection := h.projectConnectionFromConfig(c)
...
func (h *Handler) projectConnectionFromConfig(c echo.Context) ProjectConnectionInfo {
	var host string
	var port string
	var user string
	var database string
	...
	if h.Config != nil {
		parsedHost, parsedPort, parsedUser, parsedDatabase, parsedSSLMode := parseConnectionConfig(h.Config.DatabaseURL)
	...
	apiURL := ""
	if h.Config != nil {
		apiURL = strings.TrimRight(h.Config.SiteURL, "/")
	}
	if apiURL == "" {
