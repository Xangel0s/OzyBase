...
func (h *Handler) CreateRecord(c echo.Context) error {
	...
	// Insert the record
	id, err := h.DB.InsertRecord(ctx, collectionName, payload)
	if err != nil {
		return internalAPIError(c, http.StatusInternalServerError, "records.create.insert", err, "Unable to create the record right now.")
	}
...
func (h *Handler) GetRecord(c echo.Context) error {
	...
	if err != nil {
		...
		return internalAPIError(c, http.StatusInternalServerError, "records.get", err, "Unable to load the record right now.")
	}
...
func (h *Handler) UpdateRecord(c echo.Context) error {
	...
	ownerField, ownerID := h.extractRlsOwnerInfo(c)
	err := h.DB.UpdateRecord(ctx, collectionName, recordID, payload, ownerField, ownerID)
	if err != nil {
		if errors.Is(err, data.ErrRecordNotFound) {
			return c.JSON(http.StatusNotFound, map[string]string{
				"error": "record not found",
		...
		return internalAPIError(c, http.StatusInternalServerError, "records.update", err, "Unable to update the record right now.")
	}
...
func (h *Handler) DeleteRecord(c echo.Context) error {
	...
	if err != nil {
		...
		return internalAPIError(c, http.StatusInternalServerError, "records.delete", err, "Unable to delete the record right now.")
	}
	...
	if err := h.DB.BulkInsertRecord(ctx, collectionName, records); err != nil {
		...
		if errors.As(err, &importErr) {
			return c.JSON(http.StatusUnprocessableEntity, map[string]any{
				"error":      importErr.Summary,
				"error_code": "IMPORT_VALIDATION_FAILED",
				"row_errors": importErr.RowErrors,
			})
		}
		return internalAPIError(c, http.StatusInternalServerError, "records.import.bulk_insert", err, "Unable to import the provided rows right now.")
	}
